...
function Index({auth,errors,referentiels,categorieSouscripteurs,error,success}) {
    //PAGINATION

    const [categorieSouscripteursSt, setCategorieSouscripteursSt] = useState([]);
    ...
    const [pagination, setPagination] = useState({
        pageIndex:categorieSouscripteurs.current_page-1,
        pageSize:categorieSouscripteurs.per_page,
    });

    useEffect(()=>{
        setRowCount(categorieSouscripteurs.total)
        setCategorieSouscripteursSt(categorieSouscripteurs.data)
        setIsRefetching(false)
    },[categorieSouscripteurs])

    //if you want to avoid useEffect, look at the React Query example instead
    useEffect(() => {

        /*if (!categorieSouscripteursSt.length) {
            setIsLoading(true);
        } else {
        ...
        axios.post(route('superAdmin.categorieSouscripteur.paginationFiltre'),
            {
            ...
            .then(response => {
                ...
                setCategorieSouscripteursSt(response.data.data);
    ...
    const handleSubmit = () => {
        post(route('superAdmin.categorieSouscripteur.store'),{
            onSuccess:()=> {
                reset()
                setOpen(false);
            },
    ...
    const handleUpdate = () => {
        put(route('superAdmin.categorieSouscripteur.update',data.id),{
            onSuccess:()=> {
                reset()
                setOpenEdit(false);
            },
    ...
    const handleSuspend = () => {
        setOpen(false);
        router.delete(route('superAdmin.categorieSouscripteur.destroy',{id:data.id}))
    };
    ...
    return (
        <ReferentielLayout
            ...
            referentiel={'Catégories souscripteurs'}
            active={'referentiel'}
            sousActive={'superAdmin.categorieSouscripteur.index'}
            breadcrumbs={[
                {
                    text:"Catégorie souscripteur",
                    href:route("superAdmin.categorieSouscripteur.index",[auth.user.id]),
                    active:false
                },
                /*{
                    text:"Création",
                    href:route("superAdmin.categorieSouscripteur.create",[auth.user.id]),
                    active:true
                }*/
            ]}
        >
            <div className={'grid gap-5 bg-gray-200 p-2 rounded border'}>
                ...
                <div className={'flex justify-end'}>
                    <Button color={'warning'} variant={'contained'} onClick={handleClickOpen} >
                        <AddCircle className={'mr-1'}></AddCircle> Ajout catégorie de souscripteur
                    </Button>
                    ...
                    <Dialog open={open} onClose={handleClose}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Ajout de la catégorie de souscripteur</DialogTitle>
                    ...
                    <Dialog open={openEdit} onClose={handleCloseEdit}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Modification de la catégorie de souscripteur</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openShow} onClose={handleCloseShow}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Details de la catégorie de souscripteur</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openDelete} onClose={handleCloseDelete}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Suppression de la catégorie de souscripteur</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            <div className={'mt-5'}>
                                {
                                    data.message === 'delete' && 'Voulez-vous vraiment suspendre cette catégorie de souscripteur'
                                }

                                {
                                    data.message === 'check' && 'Voulez-vous vraiment débloquer cette catégorie de souscripteur'
                                }
                            </div>
