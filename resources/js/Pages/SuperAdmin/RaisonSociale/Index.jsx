...
function Index({auth,errors,referentiels,raisonsSociales,error,success}) {
    //PAGINATION

    const [raisonsSocialesSt, setRaisonsSocialesSt] = useState([]);
    ...
    const [pagination, setPagination] = useState({
        pageIndex:raisonsSociales.current_page-1,
        pageSize:raisonsSociales.per_page,
    });

    useEffect(()=>{
        setRowCount(raisonsSociales.total)
        setRaisonsSocialesSt(raisonsSociales.data)
    ...
    useEffect(() => {
        ...
        axios.post(route('superAdmin.raisonSociale.paginationFiltre'),
            {
            ...
            .then(response => {
                setRaisonsSocialesSt(response.data.data);
    ...
    const handleSubmit = () => {
        post(route('superAdmin.raisonSociale.store'),{
            onSuccess:()=> {
                reset()
                setOpen(false);
            },
    ...
    const handleUpdate = () => {
        put(route('superAdmin.raisonSociale.update',data.id),{
            onSuccess:()=> {
                reset()
                setOpenEdit(false);
            },
    ...
    const handleSuspend = () => {
        setOpen(false);
        router.delete(route('superAdmin.raisonSociale.destroy',{id:data.id}))
    };
    ...
    return (
        <ReferentielLayout
           ...
           referentiel={'Raisons sociales'}
           active={'referentiel'}
           sousActive={'superAdmin.raisonSociale.index'}
           breadcrumbs={[
               {
                   text:"Raison sociale",
                   href:route("superAdmin.raisonSociale.index",[auth.user.id]),
                   active:false
               },
               /*{
                   text:"Création",
                   href:route("superAdmin.raisonSociale.create",[auth.user.id]),
                   active:true
               }*/
           ]}
        >
            <div className={'grid gap-5 bg-gray-200 p-2 rounded border'}>
                ...
                <div className={'flex justify-end'}>
                    <Button color={'warning'} variant={'contained'} onClick={handleClickOpen} >
                        <AddCircle className={'mr-1'}></AddCircle> Ajout raison sociale
                    </Button>
                    ...
                    <Dialog open={open} onClose={handleClose}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Ajout de la raison sociale</DialogTitle>
                    ...
                    <Dialog open={openEdit} onClose={handleCloseEdit}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Modification de la raison sociale</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openShow} onClose={handleCloseShow}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Details de la raison sociale</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openDelete} onClose={handleCloseDelete}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Suppression du type de prestataire</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            <div className={'mt-5'}>
                                {
                                    data.message === 'delete' && 'Voulez-vous vraiment suspendre cette raison sociale'
                                }

                                {
                                    data.message === 'check' && 'Voulez-vous vraiment débloquer cette raison sociale'
                                }
                            </div>
