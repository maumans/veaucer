...
function Index({auth,errors,referentiels,typePrestataires,error,success}) {
    //PAGINATION
    ...
    const [pagination, setPagination] = useState({
        pageIndex:typePrestataires.current_page-1,
        pageSize:typePrestataires.per_page,
    });

    useEffect(()=>{
        setRowCount(typePrestataires.total)
        setTypeSouscripteursSt(typePrestataires.data)
    ...
    useEffect(() => {
        ...
        axios.post(route('superAdmin.typePrestataire.paginationFiltre'),
            {
    ...
    const handleSubmit = () => {
        post(route('superAdmin.typePrestataire.store'),{
            onSuccess:()=> {
                reset()
                setOpen(false);
            },
    ...
    const handleUpdate = () => {
        put(route('superAdmin.typePrestataire.update',data.id),{
            onSuccess:()=> {
                reset()
                setOpenEdit(false);
            },
    ...
    const handleSuspend = () => {
        setOpen(false);
        router.delete(route('superAdmin.typePrestataire.destroy',{id:data.id}))
    };
    ...
    return (
        <ReferentielLayout
           ...
           referentiel={'Types prestataires'}
           active={'referentiel'}
           sousActive={'superAdmin.typePrestataire.index'}
           breadcrumbs={[
               {
                   text:"Type prestataire",
                   href:route("superAdmin.typePrestataire.index",[auth.user.id]),
                   active:false
               },
               /*{
                   text:"Création",
                   href:route("superAdmin.typePrestataire.create",[auth.user.id]),
                   active:true
               }*/
           ]}
        >
            <div className={'grid gap-5 bg-gray-200 p-2 rounded border'}>
                ...
                <div className={'flex justify-end'}>
                    <Button color={'warning'} variant={'contained'} onClick={handleClickOpen} >
                        <AddCircle className={'mr-1'}></AddCircle> Ajout type de prestataire
                    </Button>
                    ...
                    <Dialog open={open} onClose={handleClose}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Ajout du type de prestataire</DialogTitle>
                    ...
                    <Dialog open={openEdit} onClose={handleCloseEdit}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Modification du type de prestataire</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openShow} onClose={handleCloseShow}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Details du type de prestataire</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openDelete} onClose={handleCloseDelete}>
                        ...
                        <DialogContent className={'space-y-5'}>
                            <div className={'mt-5'}>
                                {
                                    data.message === 'delete' && 'Voulez-vous vraiment suspendre ce type de prestataire'
                                }

                                {
                                    data.message === 'check' && 'Voulez-vous vraiment débloquer ce type de prestataire'
                                }
                            </div>
