...
function Index({auth,errors,referentiels,liens,error,success}) {
    //PAGINATION

    const [liensSt, setLiensSt] = useState([]);
    ...
    const [pagination, setPagination] = useState({
        pageIndex:liens.current_page-1,
        pageSize:liens.per_page,
    });

    useEffect(()=>{
        setRowCount(liens.total)
        setLiensSt(liens.data)
    ...
    useEffect(() => {
        ...
        axios.post(route('superAdmin.lien.paginationFiltre'),
            {
    ...
    const handleSubmit = () => {
        post(route('superAdmin.lien.store'),{
            onSuccess:()=> {
                reset()
                setOpen(false);
            },
    ...
    const handleUpdate = () => {
        put(route('superAdmin.lien.update',data.id),{
            onSuccess:()=> {
                reset()
                setOpenEdit(false);
            },
    ...
    const handleSuspend = () => {
        setOpen(false);
        router.delete(route('superAdmin.lien.destroy',{id:data.id}))
    };
    ...
    return (
        <ReferentielLayout
           ...
           referentiel={'Liens'}
           active={'referentiel'}
           sousActive={'superAdmin.lien.index'}
           breadcrumbs={[
               {
                   text:"Lien",
                   href:route("superAdmin.lien.index",[auth.user.id]),
                   active:false
               },
               /*{
                   text:"Création",
                   href:route("superAdmin.lien.create",[auth.user.id]),
                   active:true
               }*/
           ]}
        >
            <div className={'grid gap-5 bg-gray-200 p-2 rounded border'}>
                ...
                <div className={'flex justify-end'}>
                    <Button color={'warning'} variant={'contained'} onClick={handleClickOpen} >
                        <AddCircle className={'mr-1'}></AddCircle> Ajout lien
                    </Button>
                    ...
                    <Dialog open={open} onClose={handleClose}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Ajout du lien</DialogTitle>
                    ...
                    <Dialog open={openEdit} onClose={handleCloseEdit}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Modification du lien</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openShow} onClose={handleCloseShow}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Details du lien</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            {/*<DialogContentText>
                                To subscribe to this website, please enter your email address here. We
                                will send updates occasionally.
                    ...
                    <Dialog open={openDelete} onClose={handleCloseDelete}>
                        <DialogTitle className={'bg-orange-600 text-white'}>Suppression du lien</DialogTitle>
                        <DialogContent className={'space-y-5'}>
                            <div className={'mt-5'}>
                                {
                                    data.message === 'delete' && 'Voulez-vous vraiment suspendre ce lien'
                                }

                                {
                                    data.message === 'check' && 'Voulez-vous vraiment débloquer ce lien'
                                }
                            </div>
