...
function Index({auth,errors,referentiels,typeSocietes,error,success}) {
    //PAGINATION
    ...
    const [pagination, setPagination] = useState({
        pageIndex:typeSocietes.current_page-1,
        pageSize:typeSocietes.per_page,
    });

    useEffect(()=>{
        setRowCount(typeSocietes.total)
        setTypeSouscripteursSt(typeSocietes.data)
    ...
    useEffect(() => {
        ...
        axios.post(route('superAdmin.typeSocietes.paginationFiltre'),
            {
    ...
    const handleSubmit = () => {
        post(route('superAdmin.typeSocietes.store'),{
            onSuccess:()=> {
                reset()
                setOpen(false);
            },
    ...
    const handleUpdate = () => {
        put(route('superAdmin.typeSocietes.update',data.id),{
            onSuccess:()=> {
                reset()
                setOpenEdit(false);
            },
    ...
    const handleSuspend = () => {
        setOpen(false);
        router.delete(route('superAdmin.typeSocietes.destroy',{id:data.id}))
    };
    ...
    return (
        <ReferentielLayout
           ...
           active={'referentiel'}
           sousActive={'superAdmin.typeSociete.index'}
           breadcrumbs={[
               {
                   text:"Type souscripteur",
                   href:route("superAdmin.typeSociete.index",[auth.user.id]),
                   active:false
               },
               /*{
                   text:"Création",
                   href:route("superAdmin.typeSocietes.create",[auth.user.id]),
                   active:true
               }*/
           ]}
        >
