...
export default function Index({ departements, auth, success, error, errors }) {
    const columns = [
        {
            accessorKey: 'id',
            header: 'ID',
            ...
            Cell: ({ row }) => (
                <div className="flex gap-2">
                    <Tooltip title="Voir les détails">
                        <IconButton
                            color="primary"
                            onClick={() => router.get(route('admin.departement.show', [auth.user.id, row.original.id]))}
                        >
                            <VisibilityIcon />
                        </IconButton>
                    </Tooltip>
                    <Tooltip title="Modifier">
                        <IconButton
                            color="primary"
                            onClick={() => router.get(route('admin.departement.edit', [auth.user.id, row.original.id]))}
                        >
                            <EditIcon />
                        </IconButton>
                    </Tooltip>
                        ...
                        <IconButton
                            ...
                            onClick={() => {
                                if (confirm('Êtes-vous sûr de vouloir supprimer ce département ?')) {
                                    router.delete(route('admin.departement.destroy', [auth.user.id, row.original.id]));
                                }
                            }}
                        >
                            <DeleteIcon />
            ...
            <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">Gérez vos départements et leurs détails</p>
                    </div>
                    <Button
                        variant="contained"
                        startIcon={<AddIcon />}
                        onClick={() => router.get(route('admin.departement.create', [auth.user.id]))}
