...
function Index({auth,errors,produits,typeProduits,sousCategories,error,success}) {
    ...
    const handleEdit = (  el ) => {
        router.get(route("admin.stockInventaire.edit",[auth.user.id,el.id]),{preserveScroll:true})
    ...
    const handleUpdate = () => {
        put(route('admin.stockInventaire.update',data.id),{
            onSuccess:()=> {
                reset()
                setOpenEdit(false);
            },
    ...
    const handleSuspend = () => {
        setOpen(false);
        router.delete(route('admin.stockInventaire.destroy',{id:data.id}))
    };
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: 'stockGlobal', //access nested data with dot notation
                header: 'Stock global',
                //size: 10,
            },
            {
    ...
    return (
        <PanelLayout
            ...
            breadcrumbs={[
                ...
                /*{
                    text:"Création",
                    href:route("admin.stockInventaire.create",[auth.user.id]),
                    active:true
                }*/
            ]}
        >
            <div className={'grid gap-5 bg-gray-200 p-2 rounded border'}>
                ...
                <div className={'flex justify-end'}>
                    <Button color={'warning'} variant={'contained'} onClick={handleClickOpen} >
                        <AddCircle className={'mr-1'}></AddCircle> Faire un appro
                    </Button>
